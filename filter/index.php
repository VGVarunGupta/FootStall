<?php
	...
	{  
		...
		$sql=	  mysql_query("Select * from `clubs`");
		$search=  mysql_query("Select * from `leagues` where `lid`=\"".$option."\"");
		$i=1;
		$res = mysql_fetch_array($search);
		while($row = mysql_fetch_array($sql))
		{
			$var=str_split($row['cid'],3);
			if ($var[0]==$option)
			{
				$TEAM[$i]=$row['cname'];
				$LOGO[$i]=$row['crest'];
...
<body style="background: url('../images/backs/<?php echo($res['back']);?>'); background-attachment: fixed; background-size: cover;">
	...
	<div id="page">
		<h1 style="text-align:center;">Teams featuring in <?php echo $option ?></h1>
		<table cellspacing="0" style="margin:0 auto;">
			<thead>
				<tr id="attr">
					<td width="5">S.No.</td>
					<td width="2"></td>
					<td colspan=2 width="300">Team</td>
				</tr>
			</thead>
			...
			echo("
				<tr>
					<td width='5' class='oddcol'>$x</td>
					<td width='2'></td>
					<td width='50' class='evencol'><img src='$LOGO[$x]' alt='image01' width=50/></td>
					<td width='250' class='evencol'><a href='../teams/?option=$id[$x]'>$TEAM[$x]</td>
				</tr>
